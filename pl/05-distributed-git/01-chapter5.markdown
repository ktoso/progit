...
Mając już przygotowane repozytorium z którego możemy skorzystać jako "centralne" oraz potrafimy korzystać z git'a na lokalnej maszynie, najwyższy czas zajrzeć w świat rozproszonych schematów pracy.

W tym rozdziale nauczysz się pracować z Git w rozproszonym środowisku, zarówno jako "kontrybutor" jak i "integrator".
Innymi słowy nauczysz się jak kontrybuować do istniejących projektów oraz jak integrować efekty pracy rozproszonych zespołów/developerów w Twoim projekcie.

## Rozproszone schematy pracy ##

W przeciwieństwie do Scentralizowanych Systemów Kontroli Wersji (Centralized Version Control Systems (CVCSs)), rozproszona natura git'a pozwala Ci na niezwykle elastyczne schematy pracy nad projektem. W scentralizowanych systemach każdy developer pracuje na tym samym "centralnym" repozytorium, lub jego kopii. Git i jego zdecentralizowana natura od samego początku czynią z każdej kopii repozytorium na której pracuje developer zarówno "lokalne zmiany developera" jak i potencjalne repozytorium z którego ktoś inny może pobrać jego "lokalne" zmiany - każdy developer w przeciągu kilku sekund może stać się czymś na wzór "centralnego repozytorium".

Przyglądniemy się teraz kilku typowym schematom pracy oraz omówimy ich wady i zalety. Nie wszystkie z nich wykorzystują w pełni wspomnianą decentralizację w pełni. Wybranie konkretnego schematu pracy ("workflow") jednak pozostawiam Tobie to wyboru - powinieneś wybrać ten który najbardziej odpowiada twoim potrzebom oraz możliwościom. Nie bój się też eksperymentować - możesz poniższe schematy dowolnie mieszać jeżeli uznasz że taka mieszanka będzie dla Ciebie najodpowiedniejsza.

### Scentralizowany Schemat Pracy ###

W scentralizowanych systemach kontroli wersji generalnie występuje zawsze ten sam schemat - nie są wystarczająco elastyczne aby pozwolić na inne metody pracy. Sytuacja przedstawia się zatem następująco: Istnieje jedno główne repozytorium z którym wszyscy developerzy się synchronizują oraz wysyłają na nie zmiany. Pewna liczba developerów jest **FIXME** nodeami **FIXME**, wysyła tam swoje zmiany oraz pobiera zmiany pozostałych developerów (patrz Rysunek 5-1).
